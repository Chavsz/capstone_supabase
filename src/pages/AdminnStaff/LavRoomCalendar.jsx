...
const LavRoomCalendar = () => {
  ...
  useEffect(() => {
    if (!expandedAppointmentId && !hoveredAppointment) return;
    const filteredIds = new Set(
      filteredAppointments.map((appointment) => appointment.appointment_id)
    );

    if (expandedAppointmentId && !filteredIds.has(expandedAppointmentId)) {
      setExpandedAppointmentId(null);
    }

    if (
      hoveredAppointment &&
      !filteredIds.has(hoveredAppointment.appointment_id)
    ) {
